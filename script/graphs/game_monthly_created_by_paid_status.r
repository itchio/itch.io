...
png(file="game_monthly_created_by_paid_status.png", width=1000, height=700, res=120)
op <- par(mar=c(5,2,2,6), lty=0)

res <- dbGetQuery(con, "
  select
    date_trunc('month', created_at) sum_month, paid_status, count(*) as sum
  from games
  where published and not deleted and classification in (0, 2)
  group by sum_month, paid_status order by sum_month
")
...
title(main="Games uploaded monthly by paid status")
par(op)
