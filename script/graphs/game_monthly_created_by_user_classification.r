...
png(file="game_monthly_created_by_user_classification.png", width=1000, height=700, res=120)
op <- par(mar=c(5,2,2,6), lty=0)

res <- dbGetQuery(con, "
  select
    date_trunc('month', created_at) sum_month,
    user_classification as type,
    count(*) sum
  from games
  where published and not deleted and classification in (0, 2)
  group by sum_month, user_classification
  order by sum_month asc, user_classification
")
...
stops <- c(axis_stops(max(res$sum), 4, 500), total_games)
...
title(main="Games & other content created per month")
